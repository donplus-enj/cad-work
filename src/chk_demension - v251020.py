...
#INPUT_FILE = "data\\doosan\\test-001.dxf"  # 분석할 DXF 파일
#INPUT_FILE = "data\\doosan\\test-002.dxf"  # 분석할 DXF 파일
OUTPUT_REPORT = "analysis_report.txt"  # 출력 보고서 파일명

EX_LEADERS_ON = True  # 지시선이 아닌 직선 조합에서, 지시선을 찾을지 여부
...
def generate_report(doc, layers, lines, entities, texts, arrows, arrow_leaders, boundary_lines, filename):
    """분석 보고서 생성"""
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        f.write("[3. 라인 탐지 결과] :\n")
        f.write("-" * 80 + "\n")
        f.write(f"총 탐지된 라인: {len(lines)}개\n\n")
        for line in lines[:20]:  # 처음 20개만 표시
            f.write(f"ID: {line.id} {line.start_point} → {line.end_point}\n")
        f.write("\n")
        ...
        for arrow in arrows:  # 처음 10개만 표시
        ...
        # 6. 치수/주석 상세 분석
        f.write("[6. 치수/주석 상세 분석]\n")
        f.write("-" * 80 + "\n")

        # 6.1 텍스트 엔티티
        f.write(f"\n6.1 텍스트 엔티티 ({len(texts)}개)\n")
        f.write("-" * 80 + "\n")
        ...
        # 6.2 지시화살표선
        f.write(f"\n\n6.2 지시화살표선 ({len(arrow_leaders)}개)\n")
        f.write("-" * 80 + "\n")
...
def main():
    ...
    print("9단계: 보고서 생성")
    print("=" * 80)
    generate_report(doc, layers, lines, entities, texts, arrows, arrow_leaders, 
